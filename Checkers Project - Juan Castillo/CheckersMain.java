...
//					piece = createPiece(CheckersPieceType.PLAYER2KING, x, y, tokenMain1, tokenSide1);
		...
		Menu pieceMenu = new Menu("Pieces");
		Menu operationsMenu = new Menu("Operations");
			menuBar.getMenus().addAll(boardMenu, pieceMenu, operationsMenu);
	...
	private CheckersPiece createPiece(CheckersPieceType pieceType, int x, int y, Color tokenMain, Color tokenSide) {
		...
		Color kingTokenSide1 = Color.web("#171717");
		Color kingTokenSide2 = Color.web("#c6c6c6");
		...
		piece.setOnMouseReleased(e -> {
			...
			switch (results.getType()) {
				...
				CheckersPiece kingPiece = results.getPiece();
				if (checkersBoard[x - 1][newY] == checkersBoard[x - 1][1] || checkersBoard[x + 1][newY] == checkersBoard[x + 1][1] ) {
					CheckersPieceType kingPieceType = CheckersPieceType.PLAYER1KING;
					pieceGroup.getChildren().remove(piece);
					kingPiece.pieceType = kingPieceType;
					pieceGroup.getChildren().add(kingPiece);
				} else if (checkersBoard[x - 1][newY] == checkersBoard[x - 1][8] || checkersBoard[x + 1][newY] == checkersBoard[x + 1][8]) {
					// playerr 2 king me
				}
				break;
